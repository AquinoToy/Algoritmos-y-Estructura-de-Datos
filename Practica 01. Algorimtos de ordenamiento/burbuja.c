...
Autor: Equipo Guimo
    Integrantes:
        - Aldo García Ambrosio
        - Ponce Espino Miguel Angel
        - Omar Ivan Salinas Aquino
...
void BurbujaOptimizada(int *ARREGLODES, int n); // Prototipo de funcion. Recibe int * direccion del arreglo ARREGLODES, int n tamaño del arreglo
int main(int num_args, char *cadena_args[])
{
    ...
    ARREGLODES = malloc(n * sizeof(int));
    if (ARREGLODES == NULL)
    {
    ...
    for (i = 0; i < n; i++)
        scanf("%d", &ARREGLODES[i]);

    // Inicia medición del tiempo
    t_inicio = clock();

    // Llamar al algoritmo
    BurbujaOptimizada(ARREGLODES, n);
    ...
    for (i = 0; i < n; i++)
        printf("\nA[%d]=%d", i, ARREGLODES[i]);

    free(ARREGLODES);
    return 0;
}
/*
void BurbujaOptimizada(int *ARREGLODES, int n);
Descripcion: Ordena los elelementos de un arreglo ARREGLODES de tamaño n.
Recibe: int * direccion de un arreglo ARREGLODES, int n tamanio del arreglo.
Devuelve: void (No retorna valor explicito).
Observaciones: Función que orderna el arreglo ARREGLODES de menor a mayor haciendo
