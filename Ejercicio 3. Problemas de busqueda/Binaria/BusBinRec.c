/*
Implementacion del algorimto de Busqueda binaria en un conjunto ordenado
...
Compilacion:
Windows: gcc BusBinRec.c -o BusBinRec.exe
Linux/Mac: gcc BusBinRec.c -o BusBinRec
...
int Busqueda_binaria(int *A, int valor, int inicio, int fin, int *count)
{
    ...
    else
    {
        ...
        if (valor == A[mitad])
            ...
            return Busqueda_binaria(A, valor, mitad + 1, fin, count);
        else
            return Busqueda_binaria(A, valor, inicio, mitad - 1, count);
    }
}